...
function addRow() {
    ...
    let newRow = "<tr> <td onclick='selectRow(this);'><br></td><td><textarea class='form-control-sm exptable' name='description.' form='expenseForm' required maxlength='200'></textarea></td><td><select class='form-control-sm exptable' name='category.' form='expenseForm' required'>"
    ...
    newRow += "</select></td><td><input type='date' class='form-control-sm exptable' name='date.' form='expenseForm' required value='" + today + "'></td><td><select class='form-control-sm exptable' name='payer.' form='expenseForm' required><option value='Self'>Self</option>"
    ...
    newRow += "</select></td><td><input type='text' class='form-control-sm exptable' name='amount.' form='expenseForm' size='10' placeholder='$' maxlength='10' required pattern='(?=.*?\\d)^(([1-9]\\d{0,2}(\\d{3})*)|\\d+)?(\\.\\d{1,2})?$' title='Format must be currency value without dollar sign or commas e.g. 1, 2.50, 1500.75'></td></tr>"
